    ...
    <section className="py-20 bg-primary/5">
      <div className="max-w-4xl mx-auto text-center mb-16">
        ...
        <h2 className="font-display text-4xl md:text-6xl text-white uppercase mb-6 tracking-tighter">
          ¿Cómo Calculamos el <span className="text-accent">Precio</span>?
        </h2>
        <p className="font-body text-off-white/70 max-w-2xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-5xl mx-auto px-6">
        {cards.map((card) => (
          <div key={card.icon} className="border-2 border-white/10 bg-dark-navy p-10 relative overflow-hidden">
            <div className="absolute -right-4 -top-4 font-display text-7xl text-white/5 select-none">
              {card.icon}
            </div>
            <h3 className="font-display text-2xl text-accent mb-4 uppercase">{card.title}</h3>
