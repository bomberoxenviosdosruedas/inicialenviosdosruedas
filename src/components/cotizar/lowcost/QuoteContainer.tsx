...
export default function QuoteContainer() {
  ...
  const logs = [
    "DR_SYS: INICIANDO ROUTING PROGRAMADO (LOW_COST)...",
    "DR_SYS: BUSCANDO PUNTOS DE RETIRO OPTIMOS...",
    "DR_SYS: APLICANDO MATRIZ TARIFARIA LOW_COST BD...",
    "DR_SYS: RESOLVIENDO OPTIMIZACIÓN MULTI-ENTREGA...",
    "DR_SYS: COTIZACIÓN CALCULADA CON ÉXITO."
  ];
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-stretch w-full">
      ...
      <div className="lg:col-span-6 flex flex-col justify-between border-2 border-accent bg-dark-navy p-8 relative overflow-hidden group">
        ...
        <div>
          <div className="mb-8 border-b border-accent/30 pb-4">
            ...
            <p className="font-technical text-xs text-off-white/40 uppercase mt-1">
              INPUT_PARAMETERS / ECONOMY_ROUTING
            </p>
          </div>
          ...
          {!result && !calculating && (
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="space-y-2">
                <label className="font-technical text-xs text-primary uppercase tracking-widest block">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full bg-white/5 border border-white/20 pl-12 pr-4 py-4 font-body text-sm text-white focus:border-accent focus:outline-none transition-colors rounded-none placeholder:text-off-white/30"
                  />
                </div>
              </div>
              ...
              <button
                type="submit"
                className="w-full bg-accent text-dark-navy py-5 font-technical text-xl uppercase font-bold hover:bg-primary hover:text-white transition-all relative overflow-hidden group/btn flex justify-center items-center gap-3"
              >
          ...
          {calculating && (
            <div className="space-y-6 py-6">
              ...
              {/* TERMINAL EMULATOR */}
              <div className="bg-black/80 border border-accent/30 p-4 font-body text-xs text-accent-light h-48 overflow-y-auto space-y-2">
                <p className="text-white font-technical border-b border-white/10 pb-1 mb-2">ECONOMY_LOGS:</p>
                {terminalLogs.map((log, index) => (
                  <div key={index} className="flex gap-2 font-mono">
                    <span className="text-primary">{">"}</span>
                    <span className={index === terminalLogs.length - 1 ? "text-white animate-pulse" : "text-accent/80"}>
                      {log}
                    </span>
                  </div>
                ))}
          ...
          {result && (
            <div className="space-y-6">
              <div className="p-4 bg-accent/10 border border-accent/30 flex items-center gap-3">
                ...
                <div>
                  <p className="font-technical text-[10px] text-primary uppercase">Verificado</p>
                  <p className="font-body text-xs text-off-white/80">
                    Cálculo realizado con éxito basado en tarifas de entrega en el día.
                  </p>
                </div>
              </div>
        ...
        <div className="mt-8 grid grid-cols-2 gap-4 border-t border-white/10 pt-6">
          <div className="border border-white/10 p-4 bg-white/5">
            <p className="font-technical text-[10px] text-off-white/40 uppercase mb-1">Ruteo Eficiente</p>
            <p className="font-body text-xs text-accent font-bold uppercase">programado</p>
          </div>
          <div className="border border-white/10 p-4 bg-white/5">
            <p className="font-technical text-[10px] text-off-white/40 uppercase mb-1">Límite Despacho</p>
            <p className="font-body text-xs text-primary font-bold uppercase">antes de 13:00 hs</p>
          </div>
        </div>
      </div>

      {/* DYNAMIC MAP AND PRICING METRICS DISPLAY */}
      <div className="lg:col-span-6 flex flex-col justify-between border-2 border-primary bg-dark-navy p-8 relative overflow-hidden">
        ...
        <div className="h-full flex flex-col justify-between">
          <div className="mb-8 border-b border-primary/30 pb-4">
            <h3 className="font-display text-3xl text-primary uppercase tracking-tighter">
              Métricas & Ruta
            </h3>
            <p className="font-technical text-xs text-off-white/40 uppercase mt-1">
            ...
            <AnimatePresence>
              {result ? (
                <svg viewBox="0 0 400 220" className="absolute inset-0 w-full h-full z-10" fill="none">
                  ...
                  />
                  <text x="135" y="105" className="font-technical text-[9px] fill-accent font-bold">ORIGEN</text>
                  ...
                  />
                  <text x="235" y="75" className="font-technical text-[9px] fill-primary font-bold">DESTINO</text>

                  {/* Route Line connecting them */}
                  <motion.path
                    d="M 150,120 Q 200,130 220,110 T 250,90"
                    stroke="#FFD600"
                    strokeWidth="3"
          ...
          <div className="space-y-6">
            ...
            <div className="p-4 border-l-4 border-primary bg-primary/5">
              <p className="font-body text-xs text-off-white/70 leading-relaxed">
                {result 
                  ? "Esta tarifa de envío LowCost incluye recogida programada y entrega asegurada en el día (antes de las 19:00 hs) para pedidos cargados antes de las 13:00 hs."
                  : "Por favor, configure el origen y destino en el panel izquierdo para calcular el costo optimizado de tu envío."}
              </p>
            </div>
              ...
              <div className="flex gap-4">
                <a
                  href={`https://wa.me/542236602699?text=${encodeURIComponent(
                    `Hola! Quiero programar un Envío LowCost de ${origin} a ${destination}. Cotización: $${result.price} (${result.distanceKm} KM).`
                  )}`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex-1 text-center bg-primary text-white py-4 font-technical text-lg uppercase font-bold hover:bg-white hover:text-dark-navy transition-all block"
                >
                  Confirmar Envío
                </a>
              </div>
