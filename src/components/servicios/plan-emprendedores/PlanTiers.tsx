      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        {plans.map((plan, i) => (
          <div key={i} className="border-2 border-white/10 bg-white/5 p-10 flex flex-col hover:border-primary transition-all group">
             <div className="mb-8">
                <span className="bg-primary/20 text-primary border border-primary/30 font-mono text-[10px] px-2 py-1 uppercase font-bold tracking-widest mb-4 inline-block">
                  {plan.badge}
                </span>
             ...
             <div className="flex-1 space-y-4 mb-10">
                ...
                <p className="font-display text-xs text-background/50 leading-relaxed">
                   {plan.desc}
                </p>
             </div>

             <Link href="/contacto" className="w-full border border-white/20 text-white py-4 font-mono text-sm uppercase hover:bg-primary hover:border-primary transition-all">
                SOLICITAR PLAN
             </Link>
          </div>
        ))}
