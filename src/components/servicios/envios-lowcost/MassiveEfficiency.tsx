    ...
    <section className="py-24 max-w-7xl mx-auto px-6">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
        <div className="space-y-8">
           <div className="inline-block bg-secondary px-3 py-1">
             <span className="font-mono text-xs text-primary uppercase tracking-widest font-bold">LOGÍSTICA EFICIENTE</span>
           </div>
           <h2 className="font-display text-5xl md:text-7xl text-white uppercase tracking-tighter leading-[0.85]">
             MENOR COSTO, <br/>
             <span className="text-secondary">MISMA CONFIANZA</span>
           </h2>
           <p className="font-display text-lg text-background/70">
           ...
           <div className="grid grid-cols-1 sm:grid-cols-2 gap-8 pt-4">
             {[
               ...
               <div key={f.t} className="border-l-2 border-secondary pl-4">
                 <h4 className="font-mono text-white uppercase text-sm mb-1">{f.t}</h4>
                 <p className="font-display text-xs text-background/50">{f.d}</p>
               </div>
             ))}
           </div>
        </div>
        ...
        <div className="bg-white/5 border border-white/10 p-10 md:p-16">
          ...
          <div className="space-y-12 relative">
            <div className="absolute left-[15px] top-2 bottom-2 w-[1px] bg-secondary/30 -z-0" />
            {steps.map((step, i) => (
              <div key={i} className="flex gap-8 relative z-10">
                <div className="w-8 h-8 bg-primary border-2 border-secondary flex items-center justify-center shrink-0">
                   <span className="font-mono text-xs text-secondary font-bold">{i+1}</span>
                </div>
                <div>
                  <h4 className="font-display text-xl text-white uppercase mb-2">{step.title}</h4>
                  <p className="font-display text-xs text-background/60 leading-relaxed">{step.desc}</p>
                </div>
              </div>
            ))}
          </div>
