  ...
  return (
    <section className="py-24 max-w-7xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-2 gap-20 items-center">
      <div className="order-2 lg:order-1 relative">
         ...
         <div className="bg-dark-navy border-2 border-primary p-2">
            <div className="aspect-video bg-white/5 border border-white/10 flex flex-col justify-center items-center p-12 text-center">
               ...
               <p className="font-technical text-primary tracking-widest text-sm uppercase">Automatic_Integration_Active</p>
            </div>
         </div>
      </div>
      ...
      <div className="order-1 lg:order-2 space-y-12">
        <div>
          <div className="inline-block bg-primary px-3 py-1 mb-4">
            <span className="font-technical text-xs text-white uppercase tracking-widest font-bold">MERCADO ENVÍOS FLEX</span>
          </div>
        ...
        <div className="space-y-8">
          {benefits.map((b) => (
            <div key={b.id} className="flex gap-8 group">
              <div className="font-technical text-2xl text-primary font-bold group-hover:scale-125 transition-transform">{b.id}</div>
              <div className="border-b border-white/10 pb-6 flex-1">
                <h4 className="font-display text-xl text-white uppercase mb-2">{b.title}</h4>
                <p className="font-body text-sm text-off-white/60 leading-relaxed">{b.desc}</p>
              </div>
            </div>
          ))}
        </div>
