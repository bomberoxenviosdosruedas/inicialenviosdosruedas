    ...
    <section className="py-24 max-w-7xl mx-auto px-6">
      <div className="mb-16 border-l-8 border-blue-brand pl-8">
        <h2 className="font-display text-5xl md:text-7xl text-white uppercase tracking-tighter">
          TARIFAS 2026 <br/>
          <span className="text-blue-brand">ENVÍOS EXPRESS</span>
        </h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {tiers.map((tier) => (
          <div key={tier.title} className="border-2 border-white/10 p-8 flex flex-col hover:border-blue-brand transition-colors bg-white/5">
            <h3 className="font-display text-2xl text-white mb-2 uppercase">{tier.title}</h3>
            <p className="font-display text-[10px] text-white-brand/50 uppercase mb-8">{tier.desc}</p>
            
            <ul className="space-y-4 mb-10 flex-1">
              {tier.features.map((f) => (
                <li key={f} className="flex items-center gap-3 font-mono text-xs text-white-brand/80">
            ...
            <div className="pt-6 border-t border-white/10">
              <p className="font-mono text-[10px] text-blue-brand uppercase mb-1">Value_Start</p>
