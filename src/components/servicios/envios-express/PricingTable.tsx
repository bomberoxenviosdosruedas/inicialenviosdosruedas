      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {tiers.map((tier) => (
          <div key={tier.title} className="border-2 border-white/10 p-8 flex flex-col hover:border-primary transition-colors bg-white/5">
            ...
            <p className="font-body text-[10px] text-off-white/50 uppercase mb-8">{tier.desc}</p>
            
            <ul className="space-y-4 mb-10 flex-1">
              {tier.features.map((f) => (
                <li key={f} className="flex items-center gap-3 font-technical text-xs text-off-white/80">
            ...
            <div className="pt-6 border-t border-white/10">
              <p className="font-technical text-[10px] text-primary uppercase mb-1">Value_Start</p>
