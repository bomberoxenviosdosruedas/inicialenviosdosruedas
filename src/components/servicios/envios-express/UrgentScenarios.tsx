  ...
  return (
    <section className="py-24 max-w-7xl mx-auto px-6 border-t border-white/10">
      <div className="text-center mb-16">
        ...
        <p className="font-technical text-accent tracking-[0.2em] uppercase text-sm">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
        {scenarios.map((s) => (
          <div key={s.icon} className="group border border-white/10 p-8 hover:bg-primary transition-all duration-300">
            ...
            <h3 className="font-display text-2xl text-white mb-4 uppercase group-hover:text-dark-navy">
              {s.title}
            </h3>
            <p className="font-body text-xs text-off-white/60 leading-relaxed group-hover:text-dark-navy/80">
              {s.desc}
            </p>
          </div>
        ))}
