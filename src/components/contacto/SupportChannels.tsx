    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-1 mb-24">
      {channels.map((chan) => (
        <div key={chan.name} className="border border-white/10 p-12 flex flex-col hover:bg-white/5 transition-colors group">
          <div className="font-mono text-[10px] text-primary mb-8 uppercase tracking-[0.3em]">{chan.type}_CHANNEL_ACTIVE</div>
          ...
          <p className="font-display text-xs text-background/40 mb-10 flex-1 leading-relaxed">
            {chan.desc}
          </p>
          <a
            href={chan.href}
            ...
            className="w-full py-4 border-2 border-white/10 font-mono text-xs text-center uppercase font-bold tracking-widest text-white group-hover:border-primary group-hover:bg-primary transition-all"
          >
            {chan.cta}
          </a>
        </div>
