    ...
    <section className="py-24 max-w-7xl mx-auto px-6">
      <div className="flex flex-col md:flex-row justify-between items-end mb-16 gap-8">
        <div>
          ...
          <p className="font-mono text-secondary tracking-[0.2em] uppercase text-sm mt-2">
            Descubrí qué está pasando en nuestra comunidad en tiempo real.
          </p>
        </div>
        <div className="border border-white/20 px-6 py-3 font-mono text-xs text-white uppercase tracking-widest hover:bg-white hover:text-primary transition-colors cursor-pointer">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        {displayPosts.map((post) => (
          <div key={post.id} className="bg-white/5 border border-white/10 p-8 flex flex-col relative group overflow-hidden">
             <div className="absolute top-0 right-0 p-4 font-mono text-[10px] text-primary">
               {post.platform.toUpperCase()}_FEED_LIVE
             </div>
             <p className="font-mono text-[10px] text-background/40 uppercase mb-6">
               {new Date(post.timestamp).toLocaleDateString("es-AR")}
             </p>
             <p className="font-display text-sm text-background/80 leading-relaxed flex-1 mb-8">
               {post.content}
             </p>
             <div className="pt-6 border-t border-white/10 flex justify-between items-center">
                 <div className="flex items-center gap-2">
                    <div className="w-2 h-2 bg-primary rounded-brand" />
                 ...
                 <a 
                   ...
                   className="font-mono text-xs text-secondary uppercase hover:underline"
                 >
                   Ver Original
                 </a>
             </div>
