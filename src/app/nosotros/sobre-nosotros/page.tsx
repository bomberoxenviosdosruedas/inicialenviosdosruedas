      ...
      <section className="relative pt-32 pb-24 overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-6 text-center lg:text-left grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
          <div className="lg:col-span-8 space-y-8">
            <div className="inline-block bg-blue-brand px-3 py-1">
            ...
            <h1 className="font-display text-7xl md:text-9xl leading-[0.85] tracking-tighter uppercase text-white">
              LIDERANDO LA <br/>
              <span className="text-blue-brand">LOGÍSTICA URBANA</span>
            </h1>
            <p className="font-display text-xl text-white-brand/70 max-w-2xl border-l-4 border-blue-brand pl-6 py-2">
          ...
          <div className="lg:col-span-4 grid grid-cols-1 gap-4">
             {metrics.map((m, i) => (
               <div key={i} className="border border-white/10 p-6 bg-white/5 backdrop-blur-sm relative overflow-hidden group">
                  <div className="absolute top-0 left-0 w-1 h-full bg-blue-brand" />
                  <p className="font-display text-5xl text-white mb-1 group-hover:translate-x-2 transition-transform">{m.val}</p>
                  <p className="font-mono text-[10px] text-white-brand/40 uppercase tracking-widest">{m.label}</p>
               </div>
             ))}
          </div>
        </div>
      ...
      <section className="py-24 max-w-7xl mx-auto px-6">
         <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            <div className="relative">
               <div className="absolute inset-0 border-2 border-blue-brand/20 translate-x-4 translate-y-4" />
            ...
            <div className="space-y-8">
               <h2 className="font-display text-5xl text-white uppercase tracking-tighter leading-tight">Identidad Local, <br/> <span className="text-blue-brand">Alcance Profesional</span></h2>
               <p className="font-display text-white-brand/70 text-lg leading-relaxed">
               ...
               <div className="h-1 w-20 bg-blue-brand" />
            </div>
         </div>
      </section>
      ...
      <section className="py-24 max-w-7xl mx-auto px-6">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-1">
          {[
            ...
            <div key={i} className="bg-white/5 border border-white/10 p-12 group hover:bg-blue-brand transition-all duration-500">
               <h3 className="font-display text-2xl text-white uppercase mb-6 group-hover:text-blue-brand transition-colors">{item.t}</h3>
               <p className="font-display text-sm text-white-brand/60 leading-relaxed group-hover:text-blue-brand transition-colors">{item.d}</p>
            </div>
          ))}
        </div>
      </section>
